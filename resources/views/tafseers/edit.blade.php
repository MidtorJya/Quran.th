...
<form action="{{ route('tafseers.update', $tafseer->id) }}" method="post">
    ...
    <input type="number" name="youtubeId" class="form-control mb-3" placeholder="ํ" value="{{ $tafseer->datasurah_id }}"/>
    
    <strong>YoutubeId : </strong>
    <textarea class="form-control mb-3" name="datasurah_id" rows="4" placeholder="ลำดับซูเราะห์">{{ $tafseer->youtubeId}}</textarea>
